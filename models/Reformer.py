...
from layers.Embed import DataEmbedding,MDataEmbedding
...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        # Embedding
        self.enc_embedding = MDataEmbedding(configs.enc_in, configs.d_model, configs.embed, configs.freq,
                                           configs.dropout)
        # Encoder
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec,x_weather,y_weather,
                enc_self_mask=None, dec_self_mask=None, dec_enc_mask=None):
        # add placeholder
        x_enc = torch.cat([x_enc, x_dec[:, -self.pred_len:, :]], dim=1)
        x_mark_enc = torch.cat([x_mark_enc, x_mark_dec[:, -self.pred_len:, :]], dim=1)
        x_weather_enc = torch.cat([x_weather, y_weather[:, -self.pred_len:, :]], dim=1)
        # Reformer: encoder only
        enc_out = self.enc_embedding(x_enc, x_mark_enc,x_weather_enc)
