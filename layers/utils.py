...
def Myget_filter(base):

    # Create filters using pywt
    wavelet = pywt.Wavelet(base)
    H0 = np.array(wavelet.filter_bank[0])  # Low-pass filter
    G0 = np.array(wavelet.filter_bank[1])  # High-pass filter
    H1 = np.array(wavelet.filter_bank[2])  # Low-pass filter for next scale
    G1 = np.array(wavelet.filter_bank[3])  # High-pass filter for next scale
    # Normalize filter values by eliminating near-zero values
    H0[np.abs(H0) < 1e-8] = 0
    H1[np.abs(H1) < 1e-8] = 0
    G0[np.abs(G0) < 1e-8] = 0
    G1[np.abs(G1) < 1e-8] = 0
